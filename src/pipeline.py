...
def get_data(split: str, config):
    if config.get("history_length", 0) > 0:
        path = CONFIG.dataset.train_with_history(config.get("checkpoint_name", 0), split=split)
    else:
        path = CONFIG.dataset.train_no_history(config.get("checkpoint_name", 0), split=split)
...
def evaluate(model, tokenizer, val_data: datasets.Dataset, test_data: datasets.Dataset, config):
    datasets = [("val", val_data), ("test", test_data)]
    results = {}
